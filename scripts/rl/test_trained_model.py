...
def main():
    ...
    parser.add_argument(
        ...
        type=str,
        default="auto",
        help="모델 경로 (기본: auto - 최신 체크포인트 자동 선택)"
    )
    parser.add_argument(
        "--episodes",
        type=int,
    ...
    if args.model == "auto":
        # Curriculum 체크포인트 찾기
        checkpoint_dir = Path("logs/rl_training_curriculum/checkpoints")
        if checkpoint_dir.exists():
            checkpoints = sorted(checkpoint_dir.glob("roarm_ppo_curriculum_*_steps.zip"))
            if checkpoints:
                model_path = checkpoints[-1]  # 가장 최신 체크포인트
                print(f"✅ 자동 선택된 모델: {model_path}\n")
            else:
                print(f"❌ 체크포인트를 찾을 수 없습니다: {checkpoint_dir}")
